...
class _DagingEntryPageState extends State<DagingEntryPage> {
  Future<List<FoodEntry>> fetchDagingFoods(CookieRequest request) async {
    ...
    for (var d in response) {
      if (d != null) {
        ...
        if (food.fields.category.toLowerCase() == "daging") {
          listDaging.add(food);
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<FoodEntry>>(
        future: fetchDagingFoods(request),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(
              child: Text(
                'Tidak ada makanan kategori Daging tersedia.',
